#이미지에서 색 검출하는 코드에서 비디오 캡처로 바꾼 코드
import cv2 as cv
import numpy as np
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv.EVENT_LBUTTONDOWN:
        ...
        ''''''
        # 비디오에서 조명 색 때문에 색 기준을 바꾸기 위해 threshold 변수 생성
...
cv.createTrackbar('threshold', 'img_result', 0, 255, nothing) #Trackbar
...
while (True):
    ...
    kernel = np.ones((11, 11), np.uint8)  #noise 제거 코드
    img_mask = cv.morphologyEx(img_mask, cv.MORPH_OPEN, kernel)  #open
    img_mask = cv.morphologyEx(img_mask, cv.MORPH_CLOSE, kernel)  #close
